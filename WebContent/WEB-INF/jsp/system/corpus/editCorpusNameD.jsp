...
<script type="text/javascript" src="${ctx}/js/system/corpus/editCorpusNameD.js"></script>
<style type="text/css">
#but button {
	margin-bottom: 5px;
	margin-right: 5px;
...
$("#domainId").change(function() {
	...
	$.post("/AitransManager/corpus/findTranslatorLanguage.shtml?oriLanguage="+oriLanguage+"&domainId="+domainId,
		function(data){
...
function findProofread(){
	var oriLanguage=$("#oriLanguage").val();
	$.post("/AitransManager/corpus/findProofreadlanguage.shtml?oriLanguage="+oriLanguage,function(data){
...
function findAuditer(){
	var oriLanguage=$("#oriLanguage").val();
	$.post("/AitransManager/corpus/findAuditerLanguage.shtml?oriLanguage="+oriLanguage,function(data){
...
<body>
	...
	action="${pageContext.request.contextPath}/corpus/editCorpusNameD.shtml">
	<section class="panel panel-default">
		<div class="panel-body">
			<div class="form-group">
			
			<input type="hidden" name="id" value="${corpus.id}" id="corpusId">
			<input type="hidden" name="corpusTableListFormMap.oriLanguage" value="${corpus.oriLanguage}">
			
				<label class="col-sm-3 control-label">翻译类型</label>
				<div class="col-sm-9">
					<select id="languagePair" name="corpusTableListFormMap.languagePair"
						class="selectpicker show-tick form-control">
						<option value="${corpus.languagePair}">${corpus.languagePair}</option>
					</select>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
					<input type="text" class="form-control"
						placeholder="请输入语料名称" name="corpusTableListFormMap.corpusName" id="corpusName"
			...
			<div class="form-group" >
				...
				<div class="col-sm-9">
						<select id="domainId" name="corpusTableListFormMap.domainId"
							class="selectpicker show-tick form-control">
			...
			<div class="form-group" id="translator" >
				...
				<div class="col-sm-9">
						<select id="translatorId" name="corpusTableListFormMap.translatorId"
							class="selectpicker show-tick form-control">
							  <c:forEach items="${translatorList }" var="translator">
					       		<c:if test="${translator.id == corpus.translatorId}">
								<option value="${corpus.translatorId }" selected="selected">${translator.nickname}</option>
								</c:if>
								<c:if test="${translator.id !=corpus.translatorId}">
				...
				<div class="col-sm-9" >
						<select id="proofreaderId" name="proofreaderId"
					      ...
					      <c:forEach items="${proofreadList }" var="proofread">
					       		<c:if test="${proofread.id == corpus.proofreaderId }">
								<option value="${corpus.proofreaderId }" selected="selected">${proofread.nickname}</option>
								</c:if>
								</c:forEach>
						</select>
				</div>
			...
			<div class="form-group" id="auditor" >
				...
				<div class="col-sm-9">
					    ...
					    <c:forEach items="${auditerList }" var="auditer">
					       		<c:if test="${auditer.id == corpus.auditorId}">
								<option value="${corpus.auditorId }" selected="selected">${auditer.nickname}</option>
								</c:if>
								<c:if test="${auditer.id !=corpus.proofreaderId}">
			...
			<div class="form-group">
				...
				<div class="col-sm-9">
					<input type="text" class="form-control"
						placeholder="请输入客户描述" name="corpusTableListFormMap.remark" id="remark" value="${corpus.remark}">
				</div>
			</div>
