...
<body>
	...
	<section class="panel panel-default">
		<div class="panel-body">
			...
			<%-- <div class="form-group">
				<label class="col-sm-3 control-label">译员领域 </label>
				<div class="col-sm-9">
					<select class="form-control" placeholder="请选择译员" name="domain"
						id="domain">
						<!-- <option value="">请选择...</option> -->
						<c:forEach items="${domainMapperList }" var="domain">
							<option value="${domain.name }"<c:if test="${domain.name==transfer.domain }">selected="selected"</c:if> >${domain.name }</option>
						</c:forEach>
					</select>
				</div>
			</div>
			...
			<div class="line line-dashed line-lg pull-in"></div>
			<input type="hidden" name="projectFormMap.projectStateId" value="${projectflowInfo.projectStateId }">
			<%-- <div class="form-group">
				...
				<div class="col-sm-9">
					<select class="form-control" name="projectFormMap.projectStateId" id="projectState">
							<option value="3"  id="noStart"<c:if test="${projectflowInfo.projectStateId =='3' }">selected</c:if>>新建项目</option>
							<option value="4"  id="transing"<c:if test="${projectflowInfo.projectStateId =='4' }">selected</c:if>>正在翻译</option>
							<option value="5"  id="transing"<c:if test="${projectflowInfo.projectStateId =='5' }">selected</c:if>>翻译完成</option>
							<option value="6"  id="verifying" <c:if test="${projectflowInfo.projectStateId =='6' }">selected</c:if>>正在校对</option>
							<option value="7"  id="verifying" <c:if test="${projectflowInfo.projectStateId =='7' }">selected</c:if>>校对完成</option>
							<option value="8"  id="auditorying" <c:if test="${projectflowInfo.projectStateId =='8' }">selected</c:if>>正在审核</option>
							<option value="9"  id="auditorying" <c:if test="${projectflowInfo.projectStateId =='9' }">selected</c:if>>审核完成</option>
							<option value="10"  id="backing" <c:if test="${projectflowInfo.projectState =='10' }">selected</c:if>>返稿完成</option>
							<option value="11"  id="backing" <c:if test="${projectflowInfo.projectState =='11' }">selected</c:if>>等待支付</option>
							<option value="12"  id="end" <c:if test="${projectflowInfo.projectState =='12' }">selected</c:if>>支付完成</option>
							<option value="13"  id="suspend" <c:if test="${projectflowInfo.projectState =='13' }">selected</c:if>>中止</option>
					</select>
				</div>
			</div>	
			<div class="line line-dashed line-lg pull-in"></div> --%>
			...
			<div class="form-group" >
				...
				<%-- <div class="col-sm-9">
					<div style="margin-left: 100px" >
						选择路径:&nbsp;&nbsp;&nbsp;
						<input type="radio" onclick="radioClick()" id="parentPathE" name="parentPath" value="E:/" <c:if test="${parentURL=='E:/' }">checked=checked</c:if>><label for="parentPathE">E:/</label>&nbsp;&nbsp;&nbsp;
						<input type="radio" onclick="radioClick()" id="parentPathD" name="parentPath" value="C:/" <c:if test="${parentURL=='C:/' }">checked=checked</c:if>><label for="parentPathD">C:/</label>&nbsp;&nbsp;&nbsp;
				...
				<div class="col-sm-9" id="newUpload1" align="left" style="width: 240px">
					<br>
					<input type="file" style="margin-right: 10px;margin-left: 100px" id="file1" name="filename"/>
				</div>
