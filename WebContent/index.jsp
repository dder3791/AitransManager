		...
		<header class="bg-dark dk header navbar navbar-fixed-top-xs">
			<div class="navbar-header aside-md" >
				<a class="btn btn-link visible-xs"
					...
					src="${ctx}/notebook/notebook_files/logo.png" class="m-r-sm">AiTrans</a>
			...
			<ul class="nav navbar-nav navbar-right m-n hidden-xs nav-user">
				...
				<li class="dropdown">
					...
					<ul class="dropdown-menu animated fadeInRight">
						...
						<c:choose>
							<c:when test="${userFormMap.roleName eq '译员' }">
								<li><a href="#"
									onclick="javascript:transferUpdatePasswordLayer();">译员密码修改</a></li>
							</c:when>
							<c:otherwise>
								<li><a href="#"
									onclick="javascript:userUpdatePasswordLayer();">密码修改</a></li>
							</c:otherwise>
						</c:choose>
...
<script type="text/javascript">
function userUpdatePasswordLayer(){
	//加载层
	...
	layer.open({
	    ...
	    content: [rootPath+'/user/userUpdatePasswordLayer.shtml', 'no']
	});
	//关闭加载效果
	layer.close(index);
}
function transferUpdatePasswordLayer(){
	//加载层
	...
	layer.open({
	    ...
	    area: ['550px', '360px'],
	    skin: 'layui-layer-rim', //加上边框
	    content: [rootPath+'/transfer/transferUpdatePasswordLayer.shtml', 'no']
	});
	//关闭加载效果
	layer.close(index);
}
