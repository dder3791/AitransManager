...
$(function() {
	...
	grid = lyGrid({
		...
		l_column : [ {
			...
			colkey : "projectStateId",
			name : "操作",
			renderData : function(rowindex,data,rowdata,column){
				if(data!=2){
					return "<a onclick='active("+rowdata.id+")'>发布任务</a>"
				}else{
					return "任务已发布"
				}
		...
		jsonUrl : rootPath + '/project/find_Release_tasks_by_page.shtml',
		checkbox : true,
		serNumber : true
	});
...
function editFun() {
	...
	pageii = layer.open({
		...
		type : 2,
		area : [ "600px", "80%" ],
		content : rootPath + '/project/editUI.shtml?type=releaseTasks&id=' + cbox
	});
}
function addFun() {
	pageii = layer.open({
		...
		type : 2,
		area : [ "600px", "80%" ],
		content : rootPath + '/project/addUI.shtml?type=releaseTasks'
	});
}
function delFun() {
	...
	layer.confirm('是否删除？', function(index) {
		var url = rootPath + '/project/deleteEntity.shtml?type=releaseTasks';
...
function active(id){
	layer.confirm('是否发布？', function(index) {
		var url = rootPath + '/project/verify.shtml?pid='+id;
...
/*	
	pageii = layer.open({
		...
		type : 2,
		area : [ "1000px", "85%" ],
		content : rootPath + '/project/verify.shtml?id='+id
	})*/
}