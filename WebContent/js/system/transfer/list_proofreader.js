...
$(function() {
	...
	grid = lyGrid({
		...
		jsonUrl : rootPath + '/transfer/find_proofreader_by_page.shtml',
		/*checkbox : true,
		serNumber : true*/
	});
...
function addFun() {
	pageii = layer.open({
		...
		type : 2,
		area : [ "1000px", "100%" ],
		content : rootPath + '/transfer/addClientTransUI.shtml?type="校对员"'
	});
}
function delAccount() {
	var cbox = grid.getSelectedCheckbox();
...
function active(id){
	pageii = layer.open({
		...
		type : 2,
		area : [ "1000px", "85%" ],
		content : rootPath + '/transfer/proofreaderUI.shtml?id='+id+'&type=校对员'
	});
}