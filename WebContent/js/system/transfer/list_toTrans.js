		...
		l_column : [  {
			...
			renderData : function(rowindex,data,rowdata,column){
				/*if(data== "0" || data== ""){
					...
					return "<a href='javascript:active("+rowdata.id+")'>前往审核</a>";
				/*}*/
				
			}
		}
		 ],
		jsonUrl : rootPath + '/transfer/find_toTrans_by_page.shtml',
		/*checkbox : true,
		serNumber : true*/
	});
...
function addFun() {
	pageii = layer.open({
		...
		type : 2,
		area : [ "1000px", "100%" ],
		content : rootPath + '/transfer/addClientTransUI.shtml?type=翻译员'
	});
}
function delAccount() {
	var cbox = grid.getSelectedCheckbox();
...
function active(id){
	pageii = layer.open({
		...
		type : 2,
		area : [ "900px", "100%" ],
		content : rootPath + '/transfer/proofreaderUI.shtml?id='+id+'&type=翻译员'
	});
}