package com.aiTrans.controller.index;
...
import com.aiTrans.util.TreeObject;
import com.aiTrans.util.TreeUtil;
...
public class BackgroundController extends BaseController {
	...
	//2017年4月11日11:27:25 username改为accountName
	@RequestMapping(value = "login", method = RequestMethod.POST, produces = "text/html; charset=utf-8")
	public String login(String accountName, String password, HttpServletRequest request) {
		try {
			...
			String userName = userMapper.findUserName(accountName);
			String roleName = userMapper.findRoleName(accountName);
			System.out.println("----------------1--------------------"+userName);
			System.out.println("----------------2--------------------"+roleName);
			userLogin.put("userName",userName);
			userLogin.put("roleName",roleName);
	...
	public String index(Model model) throws Exception {
		...
		HashMap roleMap = (HashMap) Common.findUserSession(request);
		ResFormMap resFormMap = new ResFormMap();
		
		List<ResFormMap> mps = new ArrayList<ResFormMap>();
		
		if("译员".equals(roleMap.get("roleName"))){
			String roleId = "5";
			mps = resourcesMapper.findTransferRes(roleId);
		}else{
			resFormMap.put("userId", roleMap.get("id"));
			/*findRes(resFormMap)方法和MyRealm中的findUserResourcess(userId)方法
			 * 在本例中效果一致,查询的结果(登录用户的资源)相同,传入的参数相同(参数类型不同,一个String,一个Map)
			 * 可以看做是一个方法,
			 * 本方法用于在index.shtml页面显示登录用户的所有菜单资源,
			 * MyRealm中的方法用于验证登录用户的权限
			 */
			mps = resourcesMapper.findRes(resFormMap);
		}
		...
		// 登陆的信息回传页面
		model.addAttribute("userFormMap", roleMap);
		return "/index";
	}
	/*@RequestMapping("index")
	public String index(Model model) throws Exception {
