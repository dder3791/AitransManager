...
public class PagePlugin implements Interceptor {
    ...
    private void setPageParameter(String sql, Connection connection, MappedStatement mappedStatement,
		...
		try {
			...
			try {
				...
				countSql = "select count(1) from (" + removeOrderBys(sql)+ ") tmp_count"; 
  ...
  private static String removeOrderBys(String toSql) {  
	 ...
	 * @return
	 */
  //2017年4月26日17:04:07 去掉mysql
	public static String generatePagesSql(String sql, PageView page) {
		if (page != null) {
			if("SQLServer2008".equals(dialect)){
				System.out.println("--------com.aiTrans.plugin------------PagePlugin.java----------------------------------");
