...
public class SQLServer2008Dialect {
	...
	public String getTotalString(String sql) {
		/*toLowerCase()方法会把SQL全变为小写(比如createTime变为createtime),
		 * 这就跟数据库字段不一致,虽然也能查出来,但展示页面的js中必须将column也改为相应的小写
		 * 虽然也能正常显示,但造成另外一个问题:
		 * 导出Excel将不能导出这些和数据库不匹配的字段
		 * 所以去掉该方法
		 * 
		 */
...
//		StringBuilder sb = new StringBuilder(sql.trim().toLowerCase());
	...
	protected static void insertRowNumberFunction(StringBuilder sql,
		...
		sql.insert(selectEndIndex, " TOP 100 PERCENT ROW_NUMBER() OVER (" + orderby
				+ ") as __liud_row_nr__,");
	}
