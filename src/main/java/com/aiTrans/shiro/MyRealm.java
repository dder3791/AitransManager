...
import com.aiTrans.mapper.ResourcesMapper;
...
public class MyRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
		...
		if (loginName != null) {
			...
			/*userId为登录用户的id,
			 * 如果是管理员登录,则为userD表中的id
			 * 如果是译员登录,则为transferD中的id
			 */
			String userId = SecurityUtils.getSubject().getSession().getAttribute("userSessionId").toString();
			
			/*List<TranslatorFormMap> transferList = transferMapper.findByAttribute("id", userId, TranslatorFormMap.class);
			//roleName用于确定登录人员的身份
			String roleName = (String) transferList.get(0).get("roleName");
			
			if("译员".equals(roleName)){
				//译员登录后的资源,在角色表中译员的身份id固定为5
				String roleId = "5";
				List<ResFormMap> res = resourcesMapper.findTransferRes(roleId);
				for (ResFormMap resFormMap : res) {
					info.addStringPermission(resFormMap.get("resKey").toString());
				}
			}else{*/
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		TranslatorFormMap transferFormMap = new TranslatorFormMap();
		transferFormMap.put("accountName", "" + username + "");
		List<TranslatorFormMap> transferFormMaps = transferMapper.findByNames(transferFormMap);
		...
		}else if(transferFormMaps.size() != 0){
			SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(username, // 用户名
					transferFormMaps.get(0).get("password"), // 密码
					ByteSource.Util.bytes(username + "" + transferFormMaps.get(0).get("credentialsSalt")),// salt=username+salt
					getName() // realm name
			);
			Session session = SecurityUtils.getSubject().getSession();
			session.setAttribute("userSession", transferFormMaps.get(0));
			session.setAttribute("userSessionId", transferFormMaps.get(0).get("id"));
			return authenticationInfo;
		} else {
