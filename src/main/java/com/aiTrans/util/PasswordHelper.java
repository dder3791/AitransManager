...
public class PasswordHelper {
	...
	public void encryptPassword(TranslatorFormMap transferFormMap) {
		String salt=randomNumberGenerator.nextBytes().toHex();
		transferFormMap.put("credentialsSalt", salt);
		String newPassword = new SimpleHash(algorithmName, transferFormMap.get("password"), ByteSource.Util.bytes(transferFormMap.get("accountName")+salt), hashIterations).toHex();
		transferFormMap.put("password", newPassword); 
	}
}
