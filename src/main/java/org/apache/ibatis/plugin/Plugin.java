...
import com.aiTrans.plugin.PagePlugin;
import com.aiTrans.plugin.PageView;
import com.aiTrans.util.Common;
import com.aiTrans.util.EhcacheUtils;
...
//				if (!"ly_table".equals(entry.getKey()) && null != entry.getValue() && !"_t".equals(entry.getKey()))
			...
			for (String string : fe) {
				Object v = formMap.get(string);
				if (null != v && !StringUtils.isBlank(v.toString())) {
//					String key = mapfield.get(Configuration.COLUMN_KEY).toString();//用于识别主键
					String key = "id";
			...
			if (null != formMaps && formMaps.size() > 0) {
				//2017年4月27日10:27:26 去掉ly_后不起作用(TABLE不会正常显示--变蓝)
